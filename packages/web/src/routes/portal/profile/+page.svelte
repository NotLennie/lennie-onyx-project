...
<div class="max-w-lg">
  ...
  <div class="flex items-center gap-5 mb-8 p-5 rounded-xl" style="background-color: var(--color-surface); border: 1px solid var(--color-border)">
    <div class="w-16 h-16 rounded-full overflow-hidden flex-shrink-0 flex items-center justify-center"
      style="background-color: rgba(201,168,76,0.2)">
      {#if data.profile?.profilePictureUrl}
        <img src={data.profile.profilePictureUrl} alt="Profile" class="w-full h-full object-cover" />
      {:else}
        <span class="text-2xl font-bold" style="color: var(--color-gold)">
    ...
    <div>
      ...
      <label class="mt-1 cursor-pointer text-sm font-medium" style="color: var(--color-gold)">
      ...
      {#if pictureError}
        <p class="text-xs mt-1" style="color: #ef4444">{pictureError}</p>
      {/if}
    </div>
  </div>
  ...
  <div class="rounded-xl p-5" style="background-color: var(--color-surface); border: 1px solid var(--color-border)">
    ...
    {/if}
    {#if saveSuccess}
      <div class="mb-4 p-3 rounded-lg text-sm" style="background-color: rgba(34,197,94,0.1); color: #22c55e; border: 1px solid rgba(34,197,94,0.3)">
        {saveSuccess}
      </div>
    {/if}
    ...
    <div class="space-y-4">
      <div>
        ...
        <input
          ...
          type="text"
          bind:value={name}
          class="w-full px-4 py-2.5 rounded-lg text-white text-sm outline-none focus:ring-1"
          style="background-color: var(--color-bg); border: 1px solid var(--color-border); focus-ring-color: var(--color-gold)"
        />
      </div>

      <div>
        <label class="block text-sm font-medium text-gray-300 mb-1.5" for="email">Email Address</label>
        <input
          id="email"
          type="email"
          bind:value={email}
          class="w-full px-4 py-2.5 rounded-lg text-white text-sm outline-none"
      ...
      <div style="border-top: 1px solid var(--color-border); padding-top: 1rem; margin-top: 1rem">
        <p class="text-sm text-gray-400 mb-3">Change password — leave blank to keep current password</p>
        <div class="space-y-3">
          <input
            type="password"
      ...
      <button
        onclick={saveProfile}
        disabled={saving}
        class="w-full py-2.5 rounded-lg font-semibold text-sm transition-opacity disabled:opacity-50 mt-2"
